"""OpenAI provider implementation."""

import json
from collections.abc import AsyncIterator

import httpx

from ..provider import Provider
from ..types import (
    AssistantMessageEvent,
    Content,
    JsonObject,
    JsonValue,
    Message,
    Role,
    TextContent,
    Tool,
    ToolCall,
    Usage,
)


class OpenAIProvider(Provider):
    """OpenAI LLM provider."""

    def __init__(
        self, api_key: str, base_url: str = "https://api.openai.com/v1"
    ) -> None:
        self.api_key = api_key
        self.base_url = base_url.rstrip("/")

    def convert_tool_message(self, message: Message) -> dict[str, object]:
        tool_content = "\n".join(self.text_values(message))
        payload: dict[str, object] = {
            "role": "tool",
            "content": tool_content,
        }
        if message.tool_call_id:
            payload["tool_call_id"] = message.tool_call_id
        return payload

    def convert_non_tool_message(
        self,
        message: Message,
    ) -> dict[str, object] | None:
        parts = self.text_blocks(message)
        if not parts and not message.tool_calls:
            return None
        openai_content: object = parts[0]["text"] if len(parts) == 1 else parts
        payload: dict[str, object] = {
            "role": message.role.value,
            "content": openai_content,
        }
        if message.tool_calls:
            payload["tool_calls"] = [
                {
                    "id": tool_call.id,
                    "type": "function",
                    "function": {
                        "name": tool_call.function.get("name", ""),
                        "arguments": tool_call.function.get("arguments", "{}"),
                    },
                }
                for tool_call in message.tool_calls
            ]
        return payload

    def _convert_tools(self, tools: list[Tool]) -> list[dict[str, object]]:
        """Convert tools to OpenAI format."""
        return [
            {
                "type": "function",
                "function": {
                    "name": tool.name,
                    "description": tool.description,
                    "parameters": tool.parameters,
                },
            }
            for tool in tools
        ]

    def stream(
        self,
        model: str,
        system_prompt: str,
        messages: list[Message],
        tools: list[Tool],
    ) -> AsyncIterator[AssistantMessageEvent]:
        """Stream assistant messages from OpenAI."""
        return self._stream_impl(model, system_prompt, messages, tools)

    async def _stream_impl(
        self,
        model: str,
        system_prompt: str,
        messages: list[Message],
        tools: list[Tool],
    ) -> AsyncIterator[AssistantMessageEvent]:
        headers = {
            "Authorization": f"Bearer {self.api_key}",
            "content-type": "application/json",
        }
        payload: dict[str, object] = {
            "model": model,
            "messages": [
                {"role": "system", "content": system_prompt},
                *self.convert_messages(messages),
            ],
            "max_tokens": 4096,
            "stream": True,
        }
        if tools:
            payload["tools"] = self._convert_tools(tools)

        partial_tool_calls: dict[int, ToolCall] = {}
        async with (
            httpx.AsyncClient() as client,
            client.stream(
                "POST",
                f"{self.base_url}/chat/completions",
                headers=headers,
                json=payload,
            ) as response,
        ):
            response.raise_for_status()
            async for line in response.aiter_lines():
                if not line.startswith("data: ") or line == "data: [DONE]":
                    continue
                for event in self._parse_event(line[6:], partial_tool_calls):
                    yield event

    def _parse_event(  # noqa: C901, PLR0912
        self,
        payload: str,
        partial_tool_calls: dict[int, ToolCall],
    ) -> list[AssistantMessageEvent]:
        payload_obj: JsonValue = json.loads(payload)
        if not isinstance(payload_obj, dict):
            return []
        payload_dict: JsonObject = payload_obj

        choices_obj = payload_dict.get("choices")
        if not isinstance(choices_obj, list) or not choices_obj:
            usage_event = self._parse_usage(payload_dict)
            return [usage_event] if usage_event is not None else []
        first_choice = choices_obj[0]
        if not isinstance(first_choice, dict):
            usage_event = self._parse_usage(payload_dict)
            return [usage_event] if usage_event is not None else []

        events: list[AssistantMessageEvent] = []
        delta_obj = first_choice.get("delta")
        if isinstance(delta_obj, dict):
            content_obj = delta_obj.get("content")
            if isinstance(content_obj, str) and content_obj:
                content: list[Content] = [TextContent(type="text", text=content_obj)]
                events.append(
                    AssistantMessageEvent(
                        delta=Message(role=Role.ASSISTANT, content=content)
                    )
                )
            tool_calls_obj = delta_obj.get("tool_calls")
            if isinstance(tool_calls_obj, list):
                for tool_call_obj in tool_calls_obj:
                    if not isinstance(tool_call_obj, dict):
                        continue
                    index_obj = tool_call_obj.get("index")
                    if not isinstance(index_obj, int):
                        continue
                    partial = partial_tool_calls.get(
                        index_obj,
                        ToolCall(id="", function={"name": "", "arguments": ""}),
                    )
                    id_obj = tool_call_obj.get("id")
                    if isinstance(id_obj, str):
                        partial.id = id_obj
                    function_obj = tool_call_obj.get("function")
                    if isinstance(function_obj, dict):
                        name_obj = function_obj.get("name")
                        if isinstance(name_obj, str):
                            partial.function["name"] = name_obj
                        arguments_obj = function_obj.get("arguments")
                        if isinstance(arguments_obj, str):
                            partial.function["arguments"] = (
                                partial.function.get("arguments", "") + arguments_obj
                            )
                    partial_tool_calls[index_obj] = partial
                    events.append(
                        AssistantMessageEvent(
                            delta=Message(
                                role=Role.ASSISTANT,
                                content=[],
                                tool_calls=[
                                    ToolCall(
                                        id=partial.id,
                                        function={
                                            "name": partial.function.get("name", ""),
                                            "arguments": partial.function.get(
                                                "arguments", ""
                                            ),
                                        },
                                    )
                                ],
                            )
                        )
                    )

        finish_reason_obj = first_choice.get("finish_reason")
        if isinstance(finish_reason_obj, str) and finish_reason_obj:
            if finish_reason_obj == "tool_calls":
                finish_reason_obj = "toolUse"
            events.append(AssistantMessageEvent(finish_reason=finish_reason_obj))

        usage_event = self._parse_usage(payload_dict)
        if usage_event is not None:
            events.append(usage_event)
        return events

    def check_model_access(self, model: str) -> tuple[bool, str | None]:
        """Check model availability using provider model endpoint."""
        headers = {"Authorization": f"Bearer {self.api_key}"}
        url = f"{self.base_url}/models/{model}"
        try:
            with httpx.Client() as client:
                response = client.get(url, headers=headers)
                response.raise_for_status()
        except httpx.HTTPError as exc:
            return False, str(exc)
        return True, None

    def _parse_usage(self, payload_obj: JsonObject) -> AssistantMessageEvent | None:
        usage_obj = payload_obj.get("usage")
        if not isinstance(usage_obj, dict):
            return None
        prompt_tokens = usage_obj.get("prompt_tokens")
        completion_tokens = usage_obj.get("completion_tokens")
        total_tokens = usage_obj.get("total_tokens")
        if (
            isinstance(prompt_tokens, int)
            and isinstance(completion_tokens, int)
            and isinstance(total_tokens, int)
        ):
            return AssistantMessageEvent(
                usage=Usage(
                    input_tokens=prompt_tokens,
                    output_tokens=completion_tokens,
                    total_tokens=total_tokens,
                )
            )
        return None
